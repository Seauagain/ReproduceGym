...
ReproduceGym's own sandbox, so the host MUST reclaim them even when the run errors or
times out. ``run_guarded`` wraps :func:`reproducegym.sandbox.runner.run` in a
...
from reproducegym.compute.providers import CliRunner
...
def reclaim(runtime, *, runner: CliRunner | None = None) -> dict[str, list[str]]:
    ...
    for provider in getattr(runtime, "providers", []):
        ...
        try:
            ...
            sys.stderr.write(
                f"[reproducegym] WARNING: teardown of {provider.name} failed: {exc!r}; "
