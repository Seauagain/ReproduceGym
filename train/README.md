...
A ReproGym task dir is byte-for-byte consumable by the existing rollout in
`../../RL/ClawGym-Agents/RL/clawgym_rl_rollout.py`. The only glue:

1. `reprogym.dataset.build_dataset` / `rollout_adapter.as_rollout_source`
   flattens selected `sandboxes/<paper>/tasks/<claim>/` into a flat
